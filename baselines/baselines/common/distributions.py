...
class CategoricalPd(Pd):
    ...
    def kl(self, other):
        ...
        a1 = other.logits - \
            tf.reduce_max(other.logits, axis=-1, keep_dims=True)
        ea0 = tf.exp(a0)
        ea1 = tf.exp(a1)
...
class DiagGaussianPd(Pd):
    ...
    def renyi(self, other, alpha=2.):
        tol = 1e-45
        assert isinstance(other, DiagGaussianPd)
        var_alpha = alpha * tf.square(other.std) + \
            (1. - alpha) * tf.square(self.std)
        return alpha/2. * tf.reduce_sum(tf.square(self.mean - other.mean) / (var_alpha + tol), axis=-1) - \
            1./(2*(alpha - 1)) * (tf.log(tf.reduce_prod(var_alpha, axis=-1) + tol) -
                                  tf.log(tf.reduce_prod(
                                      tf.square(self.std), axis=-1) + tol) * (1-alpha)
...
class MultiGaussianVectorPd(Pd):
    ...
    def renyi(self, other, alpha=2.):
        ...
        renyis = []
        for selfmean, selflogstd, othermean, otherlogstd in zip(self.means, self.logstds, other.means, other.logstds):
            var_alpha = alpha * tf.exp(2*otherlogstd) + \
                (1. - alpha) * tf.exp(2*selflogstd)
            renyis.append(alpha/2. * tf.reduce_sum(tf.square(selfmean - othermean) / (var_alpha + tol), axis=-1) -
                          1./(2*(alpha - 1)) * (tf.log(tf.reduce_prod(var_alpha, axis=-1) + tol) -
                                                tf.log(tf.reduce_prod(
                                                    tf.exp(2*selflogstd), axis=-1) + tol) * (1-alpha)
...
class CholeskyGaussianPd(Pd):
    ...
    def __init__(self, flat, size):
        ...
        if size*(size+3) != 2*l:
            raise ValueError(
                'Multivariate Gaussian: parameter size does not match dimension')
        self.size = size
        ...
        self.log_det_cov = 2 * \
            tf.reduce_sum(tf.log(tf.matrix_diag_part(self.std)))
    ...
    def kl(self, other):
        ...
        half_quadratic = tf.matrix_triangular_solve(
            other.std, delta, lower=False)
        quadratic = tf.matmul(half_quadratic, half_quadratic, transpose_a=True)
...
def test_probtypes():
    ...
    diag_gauss = DiagGaussianPdType(
        pdparam_diag_gauss.size // 2)  # pylint: disable=E1101
    validate_probtype(diag_gauss, pdparam_diag_gauss)

    pdparam_categorical = np.array([-.2, .3, .5])
    categorical = CategoricalPdType(
        pdparam_categorical.size)  # pylint: disable=E1101
    ...
    bernoulli = BernoulliPdType(
        pdparam_bernoulli.size)  # pylint: disable=E1101
